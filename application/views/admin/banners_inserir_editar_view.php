      ...
      <form action="<?=BASE_URL_ADMIN.'/'.$router_class.'/'.$action_form.'_action'?>" class="editar_inserir_view" method="post" name="form_<?=$router_class?>" id="form_<?=$router_class?>" enctype="multipart/form-data">
        ...
        <select id="idcategoria" name="idcategoria">
        	...
        	<?php if($this->tags){
        			...
        			if(@$row->idcategoria == $tags->idtag) { $sl='selected="selected"';} else { $sl='';}?>
        	<option value="<?php echo $tags->idtag; ?>" <?php echo $sl;?>><?php echo $tags->nome;?></option>
        	<?php endforeach; } ?>
        </select>
        ...
        ?>
        <h2>Url do banner:</h2>
        <input name="link_banner" type="text" id="link_banner" size="100" value="<?php echo @$row->link_banner;?>" />
