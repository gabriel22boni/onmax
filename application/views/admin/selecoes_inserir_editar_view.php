  ...
  <tr>
    <td valign="top">
      <form action="<?=BASE_URL_ADMIN.'/'.$router_class.'/'.$action_form.'_action'?>" class="editar_inserir_view" method="post" name="form_<?=$router_class?>" id="form_<?=$router_class?>" enctype="multipart/form-data">
       ...
       <?php if(isset($row->idselecoes)){?>
<a target="_blank" href="http://www.facebook.com/sharer.php?u=<?php echo base_url();?>selecoes/<?php echo $row->$primary_key.'/'.$row->titulo_seo; ?>"><img src="<?php echo base_url().'_img/admin/share.png'; ?>" border="0" /></a> 
<?php } ?>
